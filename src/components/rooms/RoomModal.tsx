  ...
  // Helper to create an empty tenant form object
  ...
  export default function RoomModal({
    ...
    // Tenant fields are stored per-tenant in `tenant1`, `tenant2`, `tenant3`.
  ...
  const [roomFormData, setRoomFormData] = useState<Omit<Room, "id">>({
    ...
    // 🔥 State to track resolved tenant location names
    const [resolvedTenantNames, setResolvedTenantNames] = useState<Record<string, { stateName: string; cityName: string }>>({});

    // 🔥 Effect to resolve tenant location names
    useEffect(() => {
      ...
      const resolveNames = async () => {
        const resolved: Record<string, { stateName: string; cityName: string }> = {};
        ...
        for (const tenant of room.tenants) {
          ...
          resolved[tenant.id] = { stateName, cityName };
        }
        
        setResolvedTenantNames(resolved);
      };
    ...
    const handleSubmit = async (t: TenantFormData) => {
      // Use tenant1 form values for quick submit
      await createTenant({
      ...
      if (room) {
        ...
        if (room.tenants.length > 0) {
          ...
          setTenant1({
            ...
            city: typeof t1.city === "string" ? t1.city : "",
            state: typeof t1.state === "string" ? t1.state : "",

    // backend doesn't provide numeric IDs here — keep null so selects work
    cityId: null,
    stateId: null,
    ...
    tokenMoney: t1.tokenMoney,
        ...
        if (room.tenants.length > 1) {
          ...
          setTenant2({
            ...
            city: typeof t2.city === "string" ? t2.city : "",
state: typeof t2.state === "string" ? t2.state : "",
            // backend doesn't provide numeric IDs here — keep null so selects work
            stateId: null,
            cityId: null,
            pincode: t2.pincode,
        ...
        if (room.tenants.length > 2) {
          ...
          setTenant3({
            ...
            city: typeof t3.city === "string" ? t3.city : "",
            state: typeof t3.state === "string" ? t3.state : "",
            stateId: null,
            cityId: null,
            pincode: t3.pincode,
    ...
    const updateTenant1 = useCallback(
      (field: keyof TenantFormData, value: string | number | null) => {
          ...
          <div key={tenant.id} className="bg-muted/50 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{resolvedTenantNames[tenant.id]?.cityName || tenant.cityName || tenant.city || "—"}</p>
              </div>

              <div>
                <p className="text-muted-foreground">State</p>
                <p className="font-medium">{resolvedTenantNames[tenant.id]?.stateName || tenant.stateName || tenant.state || "—"}</p>
              </div>
